<template>
  <div class="bili-videolist-page">
      ...
      <div class="sticky-controls-content">
          ...
          <div class="bili-channel-header">
            <div class="left-channel-actions">
              <div class="channel-tags-wrapper" v-if="allAvailableTags.length > 0">
                <el-button v-for="tag in allAvailableTags" :key="tag.id"
                  :type="searchParams.tags.includes(tag.name) ? 'primary' : ''" @click="toggleTagInSearch(tag.name)"
                  class="bili-channel-button" text size="small">
                  {{ tag.name }}
                </el-button>
              </div>
            ...
            <div class="right-channel-actions">
              <el-button @click="resetSearchAndFetch" :icon="RefreshIcon" circle class="reset-btn-circle"
                title="重置筛选和排序"></el-button>
            </div>
          </div>
        </div>
        ...
        <div class="bili-list-controls-container bili-sort-section">
          <div class="sort-by-field-group">
            <el-button v-for="option in sortByOptions" :key="option.value" @click="setSortByAndSearch(option.value)"
              :class="['sort-field-btn-direct', { 'is-active-sort': searchParams.sortBy === option.value }]" text>
              {{ option.label }}
            </el-button>
          </div>
          <div class="sort-order-and-reset-group">
            <el-button @click="setSortOrderAndSearch('asc')"
              :class="['sort-order-btn', { 'is-active-sort': searchParams.sortOrder === 'asc' }]">升序</el-button>
            <el-button @click="setSortOrderAndSearch('desc')"
              :class="['sort-order-btn', { 'is-active-sort': searchParams.sortOrder === 'desc' }]">降序</el-button>
          </div>
        </div>
      </div>
    </div>
      ...
      <el-skeleton :loading="isLoading" animated :rows="10">
        <template #default>
          <div v-if="videos.length > 0" class="bili-video-grid">
            <div v-for="video in videos" :key="video.id" class="bili-video-card" @mouseenter="startPreview(video.id)"
              @mouseleave="stopPreview(video.id)" @click="navigateToPlayer(video.id)">
              <div class="bili-card-thumbnail-container">
                <video v-if="previewingVideoId === video.id" :ref="el => { if (el) videoPlayerRefs[video.id] = el }"
                  muted loop playsinline class="bili-card-preview-player"></video>
                <el-image :src="video.thumbnail_url" fit="cover" class="bili-card-thumbnail"
                  v-show="previewingVideoId !== video.id">
                  <template #error>
                    <div class="bili-image-slot-error"><span>加载失败</span></div>
                  </template>
                  <template #placeholder>
                    <div class="bili-image-slot-placeholder"><span>加载中...</span></div>
                  </template>
                </el-image>
           ...
           <div class="bili-card-line-rating-studio">
                ...
                <div class="bili-card-actions">
                  <el-dropdown @command="handleCardCommand" trigger="click" @click.stop>
                    ...
                    <template #dropdown>
                      <el-dropdown-menu>
                        <el-dropdown-item :command="{action: 'edit', video: video}"
                          :icon="EditPenIcon">编辑</el-dropdown-item>
                        <el-dropdown-item :command="{action: 'delete', video: video}" :icon="DeleteIcon"
    ...
    <div class="bili-pagination-container" v-if="totalVideos > 0 && !isLoading">
      <el-pagination background layout="prev, pager, next, jumper, ->, total, sizes" :total="totalVideos"
        :current-page="currentPage" :page-size="pageSize" :page-sizes="[12, 24, 48, 96]"
        @current-change="handleCurrentPageChange" @size-change="handlePageSizeChange" />
    </div>

    <VideoEditModal v-model:visible="editModalVisible" :video-data="currentVideoToEdit"
...
<script setup>
  ...
  import { API_BASE_URL, isConfigLoaded } from '../services/appConfigService';
  ...
  watch(() => route.query, (newQuery, oldQuery) => {
    ...
    if (JSON.stringify(newQuery) !== JSON.stringify(oldQuery) && !isLoading.value && API_BASE_URL.value) {
      triggerSearch();
    }
  ...
  watch(() => props.initialMinRating, (newVal) => {
    ...
    searchParams.sortOrder = 'desc';
    if (API_BASE_URL.value) triggerSearch();
  });

  const fetchAllAvailableTags = async () => {
    if (!API_BASE_URL.value) return;
    isLoadingTags.value = true;
    try {
      const response = await axios.get(`${API_BASE_URL.value}/api/tags`, { params: { limit: 100 } });
  ...
  const getVideoStreamUrl = (videoId) => videoId && API_BASE_URL.value ? `${API_BASE_URL.value}/api/stream/${videoId}` : '';
  ...
  const startPreview = (videoId) => {
    if (previewingVideoId.value && previewingVideoId.value !== videoId) stopPreview(previewingVideoId.value);
    if (previewTimeoutId) clearTimeout(previewTimeoutId);
    previewTimeoutId = setTimeout(async () => {
      if (previewingVideoId.value === videoId && videoPlayerRefs[videoId] && !videoPlayerRefs[videoId].paused) return;
      ...
      if (player) {
        ...
        const onCanPlay = () => {
          player.currentTime = 0; player.playbackRate = 5.0;
          player.play()
            .then(() => { if (player.playbackRate !== 5.0) player.playbackRate = 5.0; })
            .catch(e => { if (previewingVideoId.value === videoId) stopPreview(videoId); });
          player.removeEventListener('canplay', onCanPlay); player.removeEventListener('error', onError);
        };
        const onError = (e) => {
          if (previewingVideoId.value === videoId) stopPreview(videoId);
          player.removeEventListener('canplay', onCanPlay); player.removeEventListener('error', onError);
        };
        player.addEventListener('canplay', onCanPlay); player.addEventListener('error', onError);
        player.load();
      } else { if (previewingVideoId.value === videoId) stopPreview(videoId); }
    }, 300);
  };

  const fetchVideos = async (page = currentPage.value, size = pageSize.value) => {
    if (!API_BASE_URL.value) { isLoading.value = false; return; }
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL.value}/api/videos`, { params: apiParams });
      console.log('[VideoListView fetchVideos] 收到后端响应 (原始 response.data):', JSON.parse(JSON.stringify(response.data)));
      if (response.data && Array.isArray(response.data.videos)) {
        videos.value = response.data.videos.map(video => ({ ...video, thumbnail_url: video.thumbnail_url ? `${API_BASE_URL.value}${video.thumbnail_url}` : null, added_date: video.added_date ? new Date(video.added_date) : null }));
        totalVideos.value = response.data.total_count || 0;
        currentPage.value = page; pageSize.value = size;
      } else { videos.value = []; totalVideos.value = 0; }
    } catch (error) {
  ...
  const triggerSearch = () => {
    if (!API_BASE_URL.value && isConfigLoaded.value === false) { // 如果配置还没加载完，则不触发
      return;
    }
    currentPage.value = 1;
    const query = {};
    ...
    const currentRouteName = route.name || 'VideoList';
    if (Object.keys(query).length > 0 || currentRouteName === 'VideoList') {
      if (JSON.stringify(query) !== JSON.stringify(route.query)) {
        router.replace({ name: currentRouteName, query: query });
      } else {
        fetchVideos();
      }
    } else if (Object.keys(route.query).length > 0 && currentRouteName !== 'FeaturedVideos') {
      router.replace({ name: currentRouteName, query: {} });
    } else {
      fetchVideos();
    }
  };
  ...
  const resetSearchAndFetch = () => {
    ...
    // fetchVideos(); // triggerSearch 内部会调用，或者路由watch会触发
  };
  ...
  const navigateToPlayer = (videoId) => {
    ...
    if (videoToPlay && videoToPlay.path) {
      if (window.electronApp && typeof window.electronApp.playVideoLocally === 'function') {
        window.electronApp.playVideoLocally(videoToPlay.path);
        if (API_BASE_URL.value) axios.post(`${API_BASE_URL.value}/api/videos/${videoId}/view`).catch(error => { });
  ...
  const openEditModal = (video) => {
    currentVideoToEdit.value = JSON.parse(JSON.stringify(video)); // 使用深拷贝
  ...
  const handleDeleteVideo = async (videoToDelete) => {
    ...
    try {
      ...
      isLoading.value = true;
      await axios.delete(`${API_BASE_URL.value}/api/videos/${videoToDelete.id}/to-trash`);
  ...
  const handleCardCommand = (command) => {
    if (command.action === 'edit') {
      openEditModal(command.video);
    } else if (command.action === 'delete') {
      handleDeleteVideo(command.video);
    }
  };

  onMounted(() => {
    let unwatchConfig = null; // 先声明变量

    unwatchConfig = watch(isConfigLoaded, (loaded) => {
      if (loaded) {
        if (API_BASE_URL.value && API_BASE_URL.value !== '') {
          fetchAllAvailableTags();
          triggerSearch();
        } else {
          ElMessage.error('无法获取后端API地址配置。');
          isLoading.value = false;
          isLoadingTags.value = false;
        }
        // 成功获取配置并执行完操作后，再停止 watch
        if (unwatchConfig) { // 确保 unwatchConfig 已经被赋值
          unwatchConfig();
        }
      }
    }, { immediate: true });
...
<style scoped>
  ...
  .bili-channel-button.el-button:not(.is-text):not(.is-link):hover {
    color: #00a1d6;
  ...
  .bili-channel-button.el-button--primary.is-text {
    color: #fff;
  ...
  .bili-channel-button.el-button--primary.is-text:focus {
  ...
  .sort-order-and-reset-group {
    display: flex;
    align-items: center;
    gap: 6px;
  }

  .sort-field-btn-direct,
  .sort-order-btn {
    font-size: 13px;
    color: #61666d;
    padding: 0 10px;
  ...
  .sort-field-btn-direct.is-active-sort,
  .sort-order-btn.is-active-sort {
    color: #00a1d6;
    font-weight: 500;
    background-color: #e2f3fa;
  }
  ...
  .bili-image-slot-error,
  .bili-image-slot-placeholder {
    display: flex;
...
.bili-card-meta { /* 播放次数和时间那一行 */
  ...
  .bili-video-card:hover .bili-card-actions {
    opacity: 1;
  }

  .bili-action-more-btn {
    padding: 4px;
    font-size: 16px;
    color: #9499a0;
  ...
  .bili-action-more-btn:hover {
    background-color: #e3e5e7 !important;
  }

  .bili-pagination-container {
    margin-top: 30px;
    margin-bottom: 20px;
    display: flex;
    justify-content: center;
  ...
  .bili-pagination-container :deep(.el-pagination.is-background .el-pager li:not(.is-disabled).is-active) {
    background-color: var(--el-color-primary) !important;
  }

  .bili-pagination-container :deep(.el-pagination.is-background .el-pager li:not(.is-disabled):hover) {
